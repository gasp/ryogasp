...
(function ($, document, window) {
	...
	function process(settings) {
		...
		settings.id = settings.id || $(element).attr('id'); //;
        
        if (typeof settings.href === "string") {
            settings.href = $.trim(settings.href);
        }
		if (typeof settings.id === "string") {
            settings.id = $.trim(settings.id).toString().substring(3);
        }
	...
	publicMethod.prep = function (object) {
		...
		function setPosition(s) {
			publicMethod.position(s, function () {
				...
				$title.html('<a href="/spip.php?page=document&id_document='+settings.id+'">'+settings.title+'</a>').add($loaded).show();
